    ...
    def __call__(self,img):
        ...
        for i in range(100):
            ...
            if w < img.size()[2] and h < img.size()[1]:
                ...
                mean1 = torch.mean(img[0,:,:]).item()
                mean2 = torch.mean(img[1,:,:]).item()
                mean3 = torch.mean(img[2,:,:]).item()
                img[0,y:y+h,x:x+w] = mean1
                img[1,y:y+h,x:x+w] = mean2
                img[2,y:y+h,x:x+w] = mean3
                return img
        return img
